...
class SensitiveWordFilter
{
    ...
    {
        /** @var MineCache $redis */
        $redis = \Hyperf\Support\make(MineCache::class);

        if ($redis->getSensitiveWordsCacheExist()) {
            $this->dict = $redis->getSensitiveWordsCache();
        } else if ($closure instanceof \Closure){
            $this->dict = $closure($this);
            $redis->rPushSensitiveWordsCache($this->dict);
        }
